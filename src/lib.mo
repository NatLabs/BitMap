...
module {
    ...
    public func new(init_size : Nat) : BitMap {
        ...
        {
            words = Vector.init<Nat64>(init_words, 0);
            var filled_positions = 0;
        };
    };
    ...
    func grow(bitmap : BitMap, new_size : Nat) {
        ...
        Vector.addMany(bitmap.words, additional_words_needed, 0 : Nat64);
    };
    ...
    public func set(bitmap : BitMap, position : Nat, value : Bool) {
        // Use bit operations for faster division and modulo
        // position / 64 = position >> 6
        // position % 64 = position & 63
        let pos64 = Nat64.fromNat(position);
        let row = Nat64.toNat(pos64 >> 6);
        let col = Nat64.toNat(pos64 & 63);
    ...
    public func get(bitmap : BitMap, n : Nat) : Bool {
        ...
        let row = Nat64.toNat(n64 >> 6);
        let col = Nat64.toNat(n64 & 63);
    ...
    public func row_col_to_position(row : Nat, col : Nat) : Nat {
        ...
        // Use bit operations: row * 64 = row << 6
        let row64 = Nat64.fromNat(row);
        Nat64.toNat((row64 << 6) | Nat64.fromNat(col));
    };
    ...
    public func intersectInPlace(bitmap1 : BitMap, bitmap2 : BitMap) {
        let cap1 = Vector.size(bitmap1.words);
        let cap2 = Vector.size(bitmap2.words);
        let min_words = Nat.min(cap1, cap2);
    ...
    public func unionInPlace(bitmap1 : BitMap, bitmap2 : BitMap) {
        ...
        // Grow bitmap1 if bitmap2 is larger
        if (cap2 > cap1) {
            let additional_words = Int.abs(cap2 - cap1);
            Vector.addMany(bitmap1.words, additional_words, 0 : Nat64);
        };
    ...
    public func vals(bitmap : BitMap) : Iter.Iter<Nat> {
        ...
        object {
            public func next() : ?Nat {
                if (word_index >= num_words) return null;
    ...
    public func _union_no_size_update(bitmap1 : BitMap, bitmap2 : BitMap) {
        ...
        if (cap2 > cap1) {
            let cap2_bits = cap2 * WORD_SIZE;
            grow(bitmap1, cap2_bits);
        };
